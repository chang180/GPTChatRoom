...
`app/Services/GPTService.php` 使用 `gpt-5.6-luna`（`OPENAI_MODEL`）。`ConversationContextService` 在 overflow ≥ 5 則時以 `afterResponse` 非同步小結（`config/conversation.php`），聊天請求只帶「截斷後小結 + 最近 20 則」。佈署時見 [`../docs/deployment.md`](../docs/deployment.md)。
