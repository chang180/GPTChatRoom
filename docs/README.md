...
- [`deployment.md`](deployment.md): **生產環境 pull 佈署必讀**（migration、OpenAI、小結切點、Google OAuth、**Reverb 即時廣播**）
...
- 聊天室清除（依房型 Policy）
- Reverb broadcasting / Echo（`Echo.channel` vs `Echo.private`）
...
- 佇署環境：Google OAuth 與 Reverb 多人同步需人類端到端驗證（見 [`deployment.md`](deployment.md)；本機預設不連 Reverb）
